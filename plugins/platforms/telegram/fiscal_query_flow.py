...
class FiscalQueryFlow:
    ...
    def conflicts_with_start(self, adapter, data, chat_id, thread_id, user_id):
        """Don't let another text flow consume a taxpayer, period or CAPTCHA."""
        if data not in self.START_CALLBACKS:
            return False
        if data.startswith('fq:'):
            for name in ('_portal_iva_flow', '_agip_ddjj_flow', '_pdf_security_flow'):
                flow = getattr(adapter, name, None)
                if flow is not None and flow._key(chat_id, thread_id, user_id) in flow.states:
                    return True
            return False
        return (str(chat_id), str(user_id)) in self._workflow_menu_state
