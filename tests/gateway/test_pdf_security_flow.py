...
def test_adapter_routes_security_callback_only_for_authorized_user():
    async def scenario():
        ...
        from plugins.platforms.telegram.operational_menu import OperationalMenu
        adapter._operational_menu = object.__new__(OperationalMenu)
        adapter._operational_menu.flows = {}
        adapter._operational_menu.technical_user = ""
        adapter._is_callback_user_authorized = lambda *args, **kwargs: True
        adapter._operational_menu.flows["ps"] = SimpleNamespace(callback=AsyncMock(return_value=True))
        ...
        adapter._operational_menu.flows["ps"].callback.assert_awaited_once_with(
...
def test_adapter_gives_pending_security_flow_first_access_to_pdf():
    async def scenario():
        ...
        adapter._operational_menu.flows["ps"] = SimpleNamespace(document=AsyncMock(return_value=True))
        adapter._operational_menu.flows["bx"] = SimpleNamespace(document=AsyncMock(return_value=True))
        adapter._operational_menu.flows["px"] = SimpleNamespace(document=AsyncMock(return_value=True))
        message = SimpleNamespace(document=SimpleNamespace(), from_user=SimpleNamespace(id=2), chat=SimpleNamespace(id=1, type="private"))

        await adapter._handle_media_message(SimpleNamespace(message=message, update_id=3), SimpleNamespace())

        adapter._operational_menu.flows["ps"].document.assert_awaited_once_with(adapter, message)
        adapter._operational_menu.flows["bx"].document.assert_not_awaited()
        adapter._operational_menu.flows["px"].document.assert_not_awaited()
