...
def test_adapter_distributes_deployment_without_changing_identity(tmp_path):
    ...
    assert adapter._operational_menu.flows["px"].router_project_dir == project
    assert adapter._operational_menu.flows["bx"].project_dir == project
    assert adapter._operational_menu.flows["pi"].executor == executor
    assert adapter._operational_menu.flows["ad"].worker == project / "scripts/agip-ddjj-worker.py"
    assert adapter._operational_menu.flows["oa"].project_dir == project
    assert adapter._operational_menu.flows["oa"].arca_current == tmp_path / "mapa.json"
    for flow in (adapter._operational_menu.flows["px"], adapter._operational_menu.flows["bx"],
                 adapter._operational_menu.flows["pi"], adapter._operational_menu.flows["ad"],
                 adapter._operational_menu.flows["oa"]):
        assert flow.runtime_python == Path(sys.executable)
    admin = adapter._operational_menu.flows["oa"]
...
def test_invalid_deployment_rejected_without_fallback(settings):
    with pytest.raises(ValueError, match="CONTABOT_DEPLOYMENT_INVALID"):
        TelegramAdapter(PlatformConfig(enabled=True, token="fake-token", extra={"operational_menu": {"name": "Lea"}, "contabot": settings}))
